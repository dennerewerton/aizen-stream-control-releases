...
def verify_tikfinity_chatbot_payload() -> None:
    ...
    check(
        ...
        "data do General.Custom deve conter o pacote sendChatbotMessage como objeto",
    )
    ...
    check(
        first_attempt_payload.get("event") == {"source": "General", "type": "Custom"},
        "primeira tentativa deve envelopar General.Custom",
    )
    check(
        first_attempt_payload.get("data") == tikfinity_chatbot_message_payload({"message": "Boa tarde", "username": "Jarvis", "attempt": 1}),
        "primeira tentativa deve carregar sendChatbotMessage como objeto no evento oficial",
    )
    retry_payload, retry_label = tikfinity_direct_delivery_payload(
        {"message": "Boa tarde", "username": "Jarvis", "attempt": 2, "retry": True}
    )
    check(retry_label.startswith("pacote alternativo"), "reenvio deve usar o pacote direto como compatibilidade")
    check(
        retry_payload == tikfinity_chatbot_message_payload({"message": "Boa tarde", "username": "Jarvis", "attempt": 2, "retry": True}),
        "reenvio deve transmitir sendChatbotMessage direto",
    )
    bridge = Bridge()
    ...
    check(
        sent_payload.get("data") == tikfinity_chatbot_message_payload({"message": "Boa tarde", "username": "Jarvis"}),
    ...
    check(
        ...
        "retry deve transmitir o pacote direto alternativo",
    )
...
def verify_tikfinity_direct_bridge_socket_delivery() -> None:
    ...
    try:
        ...
        check(hello.get("info", {}).get("version") == "1.0.4", "ponte deve anunciar protocolo Streamer.bot atual")
        check(bool(hello.get("session")), "ponte deve enviar sessao no Hello do Streamer.bot")
        ...
        check("General.Custom" in detail, "envio direto deve relatar o evento oficial")
        ...
        official_data = official_event.get("data") or {}
        ...
        check("pacote alternativo" in retry_detail, "retry real da ponte deve usar pacote direto alternativo")
        direct_message = read_json_frame_until(sock, lambda item: item.get("action") == "sendChatbotMessage")
        check(direct_message.get("args", {}).get("message") == "Boa noite", "retry real deve carregar a mensagem no pacote direto")
    finally:
        if sock is not None:
            try:
                sock.close()
